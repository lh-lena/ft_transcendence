...
import { matchCreateInput, match, matchResponseArrayType } from '../../schemas/match';
...
export class matchMakingClass {
  ...
  private tryMultiMatch(req: matchCreateInput): match {
    let match = this.activeMatches.find((m) => m.status === 'waiting' && m.visibility === 'public');
  ...
  insert(req: matchCreateInput): match {
    ...
    match = this.activeMatches.find((m) => m.players.some((p) => p.userId === req.userId));
  ...
  join(matchId: string, req: matchCreateInput): match | undefined {
    ...
    if (match.players.length !== 1) return undefined;
