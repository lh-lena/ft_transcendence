import { PONG_CONFIG, GameSessionStatus, Direction } from "../types/game.types.js";
...
export function updateGame(state: GameState, deltaTime: number): void {
  ...
  paddleA.y = Math.max(0, Math.min(paddleA.y, PONG_CONFIG.BOARD_HEIGHT - paddleA.height - PONG_CONFIG.PADDLE_OFFSET));
  paddleB.y = Math.max(0, Math.min(paddleB.y, PONG_CONFIG.BOARD_HEIGHT - paddleB.height - PONG_CONFIG.PADDLE_OFFSET));
