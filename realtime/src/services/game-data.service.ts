...
export default function createGameDataService(app: FastifyInstance) {
  ...
  async function fetchGameData(gameId: string): Promise<StartGame> {
    app.log.debug(
      `[game-data] Fetching game data from backend. Game ID: ${gameId}`,
    );
    try {
      ...
      if (ValidationResult.success) {
        ...
        app.log.error(
          `[game-data] Invalid game data received for game ${gameId}. Validation errors: ${JSON.stringify(ValidationResult.error.issues)}`,
        );
        throw new GameError(
          `invalid game data received for game ${gameId}. try to create a new game`,
        );
      }
    } catch (error) {
      throw error;
  ...
  async function sendGameResult(result: GameResult): Promise<boolean> {
    app.log.debug(
      ...
      `[game-data] Sending game result to backend`,
    );
    try {
      ...
      app.log.debug(
        { gameId: result.gameId },
        `[game-data] Game result sent successfully`,
      );
      return true;
    } catch (error) {
      app.log.error(
        `Failed to send results of the game ${result.gameId}. ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
      return false;
    }
  }
