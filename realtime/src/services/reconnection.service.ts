...
export default function reconnectionService(app: FastifyInstance) {
  ...
  function getDisconnectionData(userId: number): DisconnectInfo | undefined {
    ...
    app.log.debug(
      `[reconnection-service] Getting disconnect info for user ${userId}: ${info ? 'found' : 'not found'}`,
    );
    return info;
  }
  ...
  async function handleDisconnect(user: User, gameId: string): Promise<void> {
    ...
    if (!game) {
      app.log.warn(
        `[reconnection-service] Cannot handle disconnection - game not found ${gameId}`,
      );
      return;
    }
    ...
    if (
      ...
      app.log.debug(
        `[reconnection-service] Disconnect ignored: already disconnected or game not active.`,
      );
      return;
    }
    ...
    app.log.debug(
      `[reconnection-service] Player ${userId} marked as disconnected in game ${gameId}`,
    );
    ...
    const timeout = setTimeout(() => {
      app.log.debug(
        `[reconnection-service] Timeout expired for user ${userId}`,
      );
    ...
    if (status !== GameSessionStatus.PAUSED) {
      ...
      app.log.debug(
        `[reconnection-service] Game ${gameId} paused due to disconnect of user ${userId}`,
      );
    } else {
      app.log.debug(
        `[reconnection-service] Game ${gameId} already paused, skipping pause`,
      );
    }
  }
  ...
  async function attemptReconnection(userId: number): Promise<string | null> {
    ...
    const newConn = app.connectionService.getConnection(userId) as
      | WSConnection
      | undefined;
    if (!newConn) {
      app.log.info(
        `[reconnection-service] Connection for user ${userId} not found`,
      );
      cleanup(userId);
      return null;
    }
  ...
  function handleReconnectionTimeout(userId: number): void {
    app.log.debug(
      `[reconnection-service] Handling reconnection timeout for user ${userId}`,
    );
    const info = disconnectedPlayers.get(userId);
    ...
    if (
      ...
      app.gameStateService
        .endGame(
          ...
          `Player ${info.username} failed to reconnect`,
        )
        .catch((error: Error | GameError) => {
          app.log.debug(
            `[reconnection-service] Failed to end game ${info.gameId} due to error: ${error?.message}`,
          );
        });
    }
    cleanup(userId);
  ...
  function setPlayerDisconnectInfo(
    ...
    app.log.debug(
      `[reconnection-service] Stored disconnect info for user ${userId}`,
    );
  }

  function cleanup(userId: number) {
    app.log.debug(
      `[reconnection-service] Cleaning up reconnection data for user ${userId}`,
    );
  ...
  app.addHook('onClose', async () => {
    app.log.debug(
      `[reconnection-service] Cleaning up reconnection service on server close`,
    );
