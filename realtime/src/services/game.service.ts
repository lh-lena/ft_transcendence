...
export default function createGameService(app: FastifyInstance) {
  async function handleStartGame(user: User, gameId: string): Promise<void> {
    ...
    try {
      const existingGameSession = gameSessionService.getGameSession(
        gameId,
      ) as GameSession;
      ...
      if (error instanceof GameError) {
        log.debug(
          `[game-service] User ID ${userId}. Game ID ${gameId}. Error: ${error.message}`,
        );
  ...
  async function createGame(user: User, gameId: string): Promise<GameSession> {
    app.log.debug(
      `[game-service] Creating game ${gameId} for user ${user.userId}`,
    );
    const { log, respond, gameSessionService } = app;
    const backendGameData = (await app.gameDataService.fetchGameData(
      gameId,
    )) as StartGame;
    if (!isExpectedPlayer(backendGameData.players, user.userId)) {
      throw new GameError(
        `server failed to create game ${gameId}. you are not an expected player`,
      );
    }
    const gameSession = await app.gameSessionService.createGameSession(
      gameId,
      backendGameData,
    );
    ...
    log.debug(
      `[game-service] Game ${gameId} created successfully for user ${user.userId}`,
    );
    return gameSession;
  }
  ...
  async function joinGame(
    ...
    app.log.debug(
      `[game-service] Handling join user ${user.userId} to game ${gameId}`,
    );
    const { log, respond, gameSessionService } = app;
    const backendGameData = (await app.gameDataService.fetchGameData(
      gameId,
    )) as StartGame;
    const isValidPlayer2 = isExpectedPlayer(
      backendGameData.players,
      user.userId,
    );
    if (!isValidPlayer2) {
      throw new GameError(`you are not an expected player for game ${gameId}`);
    }
    const isAlreadyInGame = existingGameSession.players.some(
    ...
    if (isAlreadyInGame) {
      ...
      respond.notification(
        user.userId,
        NotificationType.INFO,
        `you are already in game ${gameId}`,
      );
      assignPlayerToGame(user.userId, gameId);
      gameSessionService.setPlayerConnectionStatus(user.userId, gameId, true);
    ...
    if (!player1InBackend) {
      throw new GameError(
        `game session is invalid. please try creating a new game`,
      );
    }

    const player2FromBackend = backendGameData.players.find(
      (p) => p.userId === user.userId,
    );
    if (!player2FromBackend) {
      throw new GameError(
        `player data not found in backend for game ${gameId}`,
      );
    }
  ...
  function handleGamePause(user: User, gameId: string): void {
    ...
    app.log.debug(
      `[game-service] Handling game pause for user ${userId} in game ${gameId}`,
    );
    try {
      ...
      respond.gamePaused(
        gameId,
        `game paused by user ${user.userAlias} for ${app.config.websocket.pauseTimeout / 1000}s`,
      );
    } catch (error) {
      if (error instanceof GameError) {
        log.debug(
  ...
  async function handleGameResume(user: User, gameId: string): Promise<void> {
    ...
    app.log.debug(
      `[game-service] Handling game resume for user ${userId} in game ${gameId}`,
    );
    try {
      const game = getValidGameCheckPlayer(gameId, userId);
  ...
  function handleGameLeave(user: User, gameId: string): void {
    ...
    const { log, respond, gameStateService } = app;
    app.log.debug(
      `[game-service] Handling game leave for user ${userId} in game ${gameId}`,
    );
    try {
      ...
      const game = getValidGameCheckPlayer(gameId, userId);
      validateGameStatus(game.status, [
  ...
  function getValidGameCheckPlayer(
    gameId: string,
    userId: number,
  ): GameSession {
  ...
  function applyPlayerInputToPaddle(
    ...
    const targetPaddle = isPlayerA
      ? game.gameState.paddleA
      : game.gameState.paddleB;
  ...
  function isPlayersConnected(game: GameSession): boolean {
    const { gameId } = game;
    app.log.debug(
      `[game-service] Checking players' connection status in game ${gameId}`,
    );
  ...
  function gameReadyToStart(game: GameSession): boolean {
    ...
    if (
      game.status !== GameSessionStatus.PENDING &&
    ...
    if (game.players.length < 1) {
      app.log.debug(
        `[game-service] Cannot start game ${game.gameId} - no players in the game`,
      );
      return false;
    }
    ...
    if (!isPlayersConnected(game)) {
      app.log.debug(
        `[game-service] Cannot start game ${game.gameId} - players are not connected`,
      );
      return false;
    }
